...
10. The **only** opaque navy surface is the fixed `-z-10` backdrop div in `layout.tsx`. Every section (including Hero sticky inner) and `<body>` must stay transparent so the sitewide backdrop reads uniformly. The backdrop is composed of two stacked fixed layers: **(a) gold aurora** — three soft-light-blended radials (`rgba(212,175,55,0.14)`, `rgba(244,215,107,0.08)`, navy `#1e2d4d → #16213d`) with inset edge box-shadows creating left/right gold cinematic framing; **(b) diagonal gold hatch** — `repeating-linear-gradient` crosshatch at ~6%/4% opacity for fine texture. Both layers use **gold only** — no magenta/cyan aurora, no saffron. Do not duplicate the backdrop on individual sections. Per-section decorative overlays (chakra halo, footer horizon, scenario grid, dotted `.bg-grid-pattern`) are fine because they are compositional motifs, not background gradients.
